...
first = datetime.date(1989, 04, 16)
...
def get_page_image_url(n, retries=5):
    img_tag = get_page(n, retries=retries).find(attrs={'class':'img-comic'})
    suffix = img_tag['alt'].replace(' - Dilbert by Scott Adams', '')
    suffix = suffix.replace(' ', '_')
    return suffix, img_tag['src']
